...
const categories = [
  { id: "complexity", label: "Kompleksitas", weight: 22.22, note: "Tabel, kolom, measure, hierarchy, role, dan calculated object." },
  { id: "storage", label: "Storage", weight: 27.78, note: "USED_SIZE, dictionary, segment, dan konsentrasi baris dibandingkan dengan seluruh model." },
...
export default function ScoringSimulator() {
  ...
  return <div className="mx-auto grid max-w-7xl gap-8 px-5 py-12 sm:px-8 lg:grid-cols-[1.35fr_.65fr] lg:py-16">
    <section className="border border-[#dfe6e2] bg-white p-6 sm:p-8">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-[.15em] text-[#167d72]">Input kategori</p><h2 className="mt-2 font-[family-name:var(--font-display)] text-3xl font-extrabold">Contoh model</h2></div><button onClick={() => setValues(initial)} className="cursor-pointer border border-[#cbd6d1] bg-white px-4 py-2 text-xs font-bold hover:border-[#167d72]">Kembalikan contoh</button></div>
    ...
    <aside className="space-y-6">
      <section className="sticky top-6 bg-[#123d37] p-7 text-white shadow-[0_22px_60px_rgba(25,47,40,.16)] sm:p-9"><p className="text-xs font-extrabold uppercase tracking-[.16em] text-[#9ed1c7]">Hasil berbobot</p><div className="my-7 flex items-end gap-3"><strong className="font-[family-name:var(--font-display)] text-7xl font-extrabold leading-none">{score.toFixed(1)}</strong><span className="pb-2 text-sm text-[#bcd2cc]">/ 100</span></div><div className="flex gap-2"><span className="rounded-full px-4 py-2 text-xs font-extrabold text-white" style={{ backgroundColor: result.color }}>{result.risk}</span><span className="rounded-full border border-white/25 px-4 py-2 text-xs font-extrabold">{result.priority}</span></div><div className="mt-8 border-t border-white/15 pt-6"><p className="text-xs leading-6 text-[#cce1dc]">Formula: total <b>category score × normalized weight</b>. Operational/runtime score tidak disertakan jika evidence per database belum tersedia.</p></div></section>
      <section className="border border-[#dfe6e2] bg-white p-6"><h3 className="font-[family-name:var(--font-display)] text-lg font-extrabold">Rincian kontribusi</h3><div className="mt-4 divide-y divide-[#e7ece9]">{contributions.map(item => <div key={item.id} className="flex justify-between gap-4 py-3 text-xs"><span className="text-[#66736e]">{item.label}: {values[item.id]} × {item.weight.toFixed(2)}%</span><b>{item.contribution.toFixed(2)}</b></div>)}</div></section>
      <div className="border-l-4 border-[#e98b3b] bg-[#fff4e8] p-5 text-xs leading-6 text-[#70441f]"><b>Batas interpretasi:</b> score digunakan untuk memprioritaskan investigasi. Root cause, latency, dan manfaat tuning tetap <b>NOT PROVABLE FROM CURRENT EVIDENCE</b> sampai divalidasi.</div>
    </aside>
  </div>;
}
