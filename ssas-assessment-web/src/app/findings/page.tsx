...
export default function FindingsPage() {
  const files = fs.existsSync(findingsDirectory)
    ...
    : [];
  const findingFiles = files.filter((file) => !file.example);
  const exampleFiles = files.filter((file) => file.example);
  ...
  return <main className="min-h-screen bg-[#f6f8f5] text-[#17241f]">
    ...
    <div className="lg:ml-[210px]">
      ...
      <section className="mx-auto max-w-7xl px-5 py-12 sm:px-8 lg:py-16">
        <div className="mb-7 flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-[.15em] text-[#167d72]">Daftar file</p><h2 className="mt-2 font-[family-name:var(--font-display)] text-3xl font-extrabold">{findingFiles.length} dokumen temuan tersedia</h2></div><p className="max-w-xl text-xs leading-6 text-[#66736e]">Mencakup seluruh database yang memiliki finding di assessment. {exampleFiles.length > 0 && `${exampleFiles.length} file CONTOH PENGISIAN ditampilkan terpisah dan berisi angka ilustratif.`}</p></div>
