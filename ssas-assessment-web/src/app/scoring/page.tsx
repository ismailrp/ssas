  ...
  return <main className="min-h-screen bg-[#f6f8f5] text-[#17241f]">
    ...
    <section className="mx-auto max-w-7xl px-5 pt-12 sm:px-8">
      <div className="border border-[#dfe6e2] bg-white p-6 sm:p-8">
        ...
        <h2 className="mt-2 font-[family-name:var(--font-display)] text-3xl font-extrabold">Bobot dinormalisasi karena evidence operasional belum merata</h2>
        ...
        <p className="mt-6 border-l-4 border-[#e98b3b] bg-[#fff4e8] p-4 text-xs leading-6 text-[#70441f]"><b>Penting:</b> bobot menunjukkan kontribusi setiap kategori terhadap <i>priority score</i>, bukan terhadap latency, CPU, atau memory. Runtime hanya digunakan sebagai sinyal awal sampai workload terbukti representatif.</p>
      </div>
    </section>
    <section className="mx-auto max-w-7xl px-5 pt-8 sm:px-8">
      <div className="grid gap-6 border border-[#dfe6e2] bg-white p-6 sm:p-8 lg:grid-cols-[.8fr_1.2fr]">
        <div><p className="text-xs font-extrabold uppercase tracking-[.15em] text-[#167d72]">Perhitungan category score</p><h2 className="mt-2 font-[family-name:var(--font-display)] text-3xl font-extrabold">Contoh Relationship = 60</h2><p className="mt-4 text-sm leading-7 text-[#66736e]">Angka pada slider adalah <i>category score</i> 0–100. Pada laporan aktual, generator menghitungnya dari posisi relatif database terhadap 54 model Tabular dan indikator struktur yang ditemukan.</p></div>
        <div className="bg-[#f4f7f5] p-5 sm:p-7">
          ...
          <div className="mt-4 space-y-3 text-xs leading-6 text-[#66736e]">
            <p><b>Mengapa 70%?</b> Ini heuristic desain assessment, bukan angka yang ditemukan dari pengukuran latency. Maksimal 70 poin dialokasikan berdasarkan posisi jumlah relationship dibandingkan seluruh model, sedangkan maksimal 30 poin untuk indikator struktur bidirectional/many-to-many.</p>
